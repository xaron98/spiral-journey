...
/// All spiral-specific content extracted from SpiralTab.
/// SpiralTab is now a thin NavigationStack wrapper; this view holds all state,
/// gestures, overlays, sheets, and helper methods for the spiral chronobiograph.
struct SpiralModeView: View {
    ...
    // Undo manual episode
    @State private var lastManualEpisodeID: UUID? = nil
    ...
    @State private var hasAnimatedGrowth = false
    ...
    private var activeBody: some View {
        ...
        GeometryReader { screen in
            ZStack(alignment: .bottom) {
                ...
                // ── Layer 3: Sleep log button (DNA button removed — now in pager) ──
                VStack {
                    HStack {
                        Spacer()
                ...
                if lastManualEpisodeID != nil {
                    VStack {
                        ...
                        HStack(spacing: 12) {
                            ...
                            Text(String(localized: "spiral.undo.registered", bundle: bundle))
                                .font(.subheadline.weight(.medium))
                                .foregroundStyle(SpiralColors.text)
                            Spacer()
                            Button {
                                if let id = lastManualEpisodeID {
                                    store.removeEpisode(id: id)
                                }
                                undoDismissTask?.cancel()
                                withAnimation(.easeIn(duration: 0.2)) { lastManualEpisodeID = nil }
                            } label: {
                                Text(String(localized: "spiral.undo.button", bundle: bundle))
                        ...
                        .padding(.horizontal, 20)
                        .padding(.bottom, screen.safeAreaInsets.bottom + 90)
                        .transition(.move(edge: .bottom).combined(with: .opacity))
                    }
                    .animation(.spring(response: 0.35), value: lastManualEpisodeID != nil)
                }

                // ── Layer 8: Bottom action bar ───────────────────────────
                VStack {
                    Spacer()
                    actionBar
        ...
        .onAppear {
            ...
            // Guard: only run the growth animation once (TabView(.page) re-triggers .onAppear on swipe back).
            if !hasAnimatedGrowth {
                hasAnimatedGrowth = true
                if store.spiralRevealAnimation {
                    // Small delay to let TabView layout stabilize before starting the timer.
        ...
        .onChange(of: store.sleepEpisodes.count) { old, count in
            ...
            if count == 0 {
                ...
                // Only reset cursor to now on first import (0→N), not on manual entry.
                // Manual entry: user has the cursor where they want it.
                if wasEmpty {
                    cursorAbsHour = nowAbsHour
                }
            }
        ...
        HStack(alignment: .bottom, spacing: 24) {
            ...
            Button {
                ...
                ZStack {
                    ...
                    if store.isProcessing && lastManualEpisodeID != nil && sleepStartHour == nil && eventLoggingType == nil {
                        ProgressView()
                            .tint(.white)
                    } else {
    ...
    private func handleLogButton() {
        if sleepStartHour == nil {
            ...
            #if DEBUG
            print("[LOG] Sleep START marked at \(cursorAbsHour)")
            #endif
        } else {
            let start = min(sleepStartHour!, cursorAbsHour)
            let end   = max(sleepStartHour!, cursorAbsHour)
            #if DEBUG
            print("[LOG] Sleep END tap. sleepStartHour=\(sleepStartHour!), cursorAbsHour=\(cursorAbsHour), start=\(start), end=\(end), duration=\(end-start)")
            #endif
            if end - start >= 0.25 {
                let episode = SleepEpisode(start: start, end: end, source: .manual)
                store.addManualEpisode(episode)
                #if DEBUG
                print("[LOG] Episode registered: \(start) → \(end), episodes count=\(store.sleepEpisodes.count)")
                #endif
                let endTurns = end / store.period
                if endTurns > maxReachedTurns { maxReachedTurns = endTurns }
                // Show undo toast
                withAnimation(.easeOut(duration: 0.25)) { lastManualEpisodeID = episode.id }
                undoDismissTask?.cancel()
                undoDismissTask = Task {
                    try? await Task.sleep(for: .seconds(5))
                    guard !Task.isCancelled else { return }
                    withAnimation(.easeIn(duration: 0.25)) { lastManualEpisodeID = nil }
                }
            } else {
                #if DEBUG
                print("[LOG] Duration too short (\(end-start)h < 0.25h), NOT registered")
                #endif
            }
            sleepStartHour = nil
        }
...
/// Compact 3x2 glass-style event button grid shown below the spiral.
